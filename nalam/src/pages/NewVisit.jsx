            ...
            {showTypedFallback && (
                <div className="w-full max-w-sm mb-6">
                    ...
                    <textarea
                        ...
                        rows={4}
                        className="w-full p-3 border rounded-xl bg-white text-gray-800 outline-none mb-3"
                        placeholder="அம்மாவின் கவலை என்ன? / What is the mother's concern?"
                    />
                    <button
                        onClick={handleTypedSubmit}
                        disabled={processing || typedText.trim().length === 0}
                        className={`w-full p-3 rounded-xl font-bold text-white ${processing || typedText.trim().length === 0 ? 'bg-gray-400 cursor-not-allowed' : 'bg-brand-green active:bg-green-700'} transition`}
                    >
                        உரையை சமர்ப்பிக்கவும் / Submit typed concern
                    </button>
                </div>
            )}
